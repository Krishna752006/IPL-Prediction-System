...
PLAYER_TO_BATTER_ROLE = {p: role for role, players in BATTER_CLASSES.items() for p in players}
PLAYER_TO_BOWLER_ROLE = {p: role for role, players in BOWLER_CLASSES.items() for p in players}
...
def print_regression_metrics(name, y_true_norm, y_pred_norm, n_features=40):
    ...
    print(f"  ── Calibration ──────────────────────────────")
    print(f"  Bias         : {bias:+.2f} runs  ({'over' if bias > 0 else 'under'}-predicting)")
...
def inject_debutant_embeddings(model, train_dataset, noise_std=0.05):
    ...
    configs = [
        ("Batter", model.batter_embedding, seen_batters, set(train_dataset.player2idx.values()), idx2player, PLAYER_TO_BATTER_ROLE),
        ("Non-Striker", model.non_striker_embedding, seen_non_strikers, set(train_dataset.player2idx.values()), idx2player, PLAYER_TO_BATTER_ROLE),
        ("Bowler", model.bowler_embedding, seen_bowlers, set(train_dataset.player2idx.values()), idx2player, PLAYER_TO_BOWLER_ROLE),
        ("Venue", model.venue_embedding, seen_venues, set(train_dataset.venue2idx.values()), idx2venue, None),
        ("MatchState", model.match_state_embedding, seen_states, set(range(model.match_state_embedding.num_embeddings)), None, None)
    ]
    ...
    with torch.no_grad():
        for name, embed_layer, seen_set, all_ids, idx2name, role_map in configs:
            ...
            if role_map is not None:
                ...
                global_avg = weights[list(seen_set)].mean(dim=0) if seen_set else torch.zeros_like(weights[0])
            else:
                global_avg = weights[list(seen_set)].mean(dim=0) if seen_set else torch.zeros_like(weights[0])

            injected_count = 0
            for unseen_id in unseen_ids:
                if unseen_id == 0: continue
            ...
            print(f"  [{name}] Processed {len(seen_set)} seen | Injected {injected_count} unseen")
            
def save_static_embeddings_to_json(model, player2idx, venue2idx, target_year, save_dir="./saved_seasons"):
    ...
    embeddings_dict = {
        ...
        "venues": {},
        "season": {},
        "match_state": model.match_state_embedding.weight.detach().cpu().numpy().tolist()
    }
...
def inject_season_embedding(model, train_dataset, target_year, noise_std):
    ...
    seen_season_ids = set(
        np.unique(train_dataset.X_categorical[:, :, 4])
    )
    ...
    avg_embedding = torch.stack(
        [weights[sid] for sid in seen_season_ids]
    ).mean(dim=0)
...
def evaluate_model(model, dataloader, wicket_thresh, wide_thresh, split_name="VAL", return_score=False):
    ...
    all_wicket_preds = (np.array(all_wicket_probs) > wicket_thresh).astype(float).tolist()
    ...
    eval_r2, _, _, _ = print_regression_metrics(split_name, all_score_targets, all_score_preds)

    print_binary_metrics("WICKET", all_wicket_targets, all_wicket_preds, all_wicket_probs)
    ...
    if return_score:
        try: wicket_auc = roc_auc_score(all_wicket_targets, all_wicket_probs)
        except ValueError: wicket_auc = 0.0
        
        try: wide_auc = roc_auc_score(all_wide_targets, all_wide_probs)
        except ValueError: wide_auc = 0.0

        return (
            0.55 * max(0.0, eval_r2)
            + 0.20 * wicket_auc
            + 0.25 * wide_auc
        )
...
def train_one_year(
    ...
    wicket_criterion = nn.BCEWithLogitsLoss(
        pos_weight=wicket_pos_weight,
        reduction="none"
    )

    wide_criterion = nn.BCEWithLogitsLoss(
        pos_weight=wide_pos_weight,
        reduction="none"
    )
    ...
    for epoch in range(epochs):
        ...
        for batch in loop:
            ...
            run_loss_raw = run_criterion(outputs["score"], batch["score_target"].to(DEVICE))
            wicket_loss_raw = wicket_criterion(outputs["wicket"], batch["wicket_target"].to(DEVICE))
            wide_loss_raw = wide_criterion(outputs["wide"], batch["wide_target"].to(DEVICE))
            ...
            all_wicket_probs.extend(torch.sigmoid(outputs["wicket"]).detach().cpu().numpy())
            all_wicket_targets.extend(batch["wicket_target"].cpu().numpy())
        ...
        print(f"Adaptive thresholds → wicket:{wicket_thresh:.3f}  wide:{wide_thresh:.3f}")
        ...
        if (epoch + 1) % 2 == 0:
            ...
            r2, ccc, mae, mse = print_regression_metrics("TRAIN", all_score_targets, all_score_preds)
            ...
            print_binary_metrics("TRAIN WICKET", all_wicket_targets, all_wicket_preds, all_wicket_probs)
            print_binary_metrics("TRAIN WIDE", all_wide_targets, all_wide_preds, all_wide_probs)
            ...
            if current_composite > best_composite_score:
                ...
                if no_improve_count >= early_stop_patience:
                    print(f"  ⚠ Early stopping triggered at epoch {epoch+1} — training plateaued")
                    break
    ...
    val_preds, val_targets = evaluate_model( model, val_loader, wicket_thresh, wide_thresh, split_name="VALIDATION" )
    test_preds, test_targets = evaluate_model( model, test_loader, wicket_thresh, wide_thresh, split_name="TEST" )  
